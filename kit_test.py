...
diffeq_test = Equation(
    model,
    sp.Derivative(u_sym, t_sym)
    + u_sym * sp.Derivative(u_sym, x_sym)
    - (sp.S(1) / 31) * sp.Derivative(u_sym, x_sym, 2),
)
...
sub_exp = Expression(
    {**coords, U_sym.name: (1,)}, (sp.S(1) / 31) * sp.Derivative(U_sym, x_sym, 2)
)
...
sub_exp_test = Equation(
    model,
    sp.Derivative(U_sym, T_sym) + U_sym * sp.Derivative(U_sym, x_sym) - beta_sym,
    [(beta_sym.name, sub_exp), (U_sym.name, sub_exp_u), (T_sym.name, sub_exp_t)],
)
